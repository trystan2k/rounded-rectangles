...
```bash
# Create draft PR from issue (MANDATORY: use template at docs/templates/pull-request.md)
...
When creating a PR for this repository, **ALWAYS** use the PR template at `docs/templates/pull-request.md`.
Read the template first, then fill in each section with the actual content from the changes made.

### PR Base Branch Detection (MANDATORY)

Before creating a PR, **ALWAYS** detect which branch the working branch was created from and target the PR to that branch.

```bash
# Detect the base branch (the branch the current branch diverged from)
BASE_BRANCH=$(git merge-base --fork-point main 2>/dev/null || git merge-base --fork-point develop 2>/dev/null || echo "main")

# Verify by checking recent merge-base candidates
git log --oneline --graph --first-parent main..HEAD 2>/dev/null | head -5
git log --oneline --graph --first-parent develop..HEAD 2>/dev/null | head -5

# Create PR targeting the detected base branch
gh pr create --base "$BASE_BRANCH" --body-file docs/templates/pull-request.md
```

**Rule**: If the branch was created from `develop`, the PR must target `develop`. If created from `main`, target `main`. Never assume `main` — always detect.
...
# Create with title and body (using template)
gh pr create \
  --title "Feature: Add new functionality" \
  --body-file docs/templates/pull-request.md

# Fill body from template
gh pr create --body-file .github/PULL_REQUEST_TEMPLATE.md

# Set base branch (DETECTED, not assumed)
...
BASE="main"
# Use heredoc for exact body — MANDATORY: follow docs/templates/pull-request.md structure
BODY=$(cat <<'EOF'
## Summary
...
### Feature area
-   **New component** — `src/components/Feature/Feature.tsx`
-   **Unit tests** — `src/components/Feature/Feature.test.ts`
...
## Test coverage
-   **Tests passing**: 42/42
-   **Coverage**: 90% statements · 85% branches · 88% functions · 89% lines

### New tests
-   `src/components/Feature/Feature.test.ts` — covers rendering and interactions
